...
const profileFormSchema = z.object({
  ...
  email: z.string()
    ...
    .max(100, { message: 'L\'email non può superare i 100 caratteri' }),
  codiceFiscale: z.string()
    .regex(/^[A-Z]{6}[0-9]{2}[A-Z][0-9]{2}[A-Z][0-9]{3}[A-Z]$/, { 
      message: 'Inserisci un codice fiscale valido (formato: XXXXXX00X00X000X)' 
    })
    .optional()
    .or(z.literal('')),
});
...
export default function ProfileFormStub() {
  ...
  return (
    <div className="max-w-md mx-auto p-6 bg-white rounded-lg shadow-md">
      <h2 className="text-2xl font-bold mb-6 text-gray-800">Profilo Utente</h2>
      
      {formSuccess && formData && (
        <div className="mb-6 p-4 bg-green-100 text-green-700 rounded-md">
          <p className="font-medium">Form inviato con successo!</p>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="pt-2">
          ...
          >
            {isSubmitting ? 'Invio in corso...' : 'Salva Profilo'}
          </button>
        </div>
      </form>
    </div>
